...
$stmtAudit = $conn->prepare("
    ...
    VALUES (?, 'Visit', 'Visited Admin Audit Logs (admin_audit.php)', ?, ?, NOW())
");
...
if (!empty($search)) {
  ...
  $filterSql = "WHERE a.Action LIKE '%$safe%' OR u.Name LIKE '%$safe%'";
}
...
$totalRes = $conn->query("SELECT COUNT(*) as total 
                          ...
                          $filterSql");
$totalLogs = $totalRes->fetch_assoc()['total'];
$totalPages = ceil($totalLogs / $logsPerPage);

$res = $conn->query("SELECT a.*, u.Name AS AdminName 
                     FROM audit_logs a
                     JOIN users u ON a.AdminID = u.UserID
                     $filterSql
...
<head>
  <meta charset="utf-8">
  <title>Hydrolink - Audit Trail Page</title>
  ...
  <div class="container-fluid bg-light py-5">
    <div class="container py-5">
      <div class="text-center mx-auto pb-5 wow fadeInUp" data-wow-delay="0.2s" style="max-width: 800px;">
        <h4 class="text-uppercase text-primary">Audit Trail</h4>
        <h1 class="display-4 text-capitalize mb-3">Administrator Activity Logs</h1>
        <p>Track administrator actions such as logins, approvals, edits, and deletions.</p>
      </div>

      <!-- Search Form -->
      <form method="GET" class="mb-4">
        <div class="input-group">
          <input type="text" name="search" class="form-control" placeholder="Search by action or admin name"
      ...
      <div class="card shadow-sm">
        <div class="card-body p-0">
          <?php if (empty($logs)): ?>
            <div class="alert alert-info text-center m-3">No logs found.</div>
          <?php else: ?>
              ...
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  <?php foreach ($logs as $log): ?>
                    <tr>
                      <td><?= $log['LogID'] ?></td>
                      <td><span class="fw-bold"><?= htmlspecialchars($log['AdminName']) ?></span></td>
                      <td>
                        <span class="badge bg-<?=
                          stripos($log['Action'], 'delete') !== false ? 'danger' :
                          (stripos($log['Action'], 'edit') !== false ? 'warning' :
                            (stripos($log['Action'], 'approve') !== false ? 'success' : 'secondary'))
                          ?>">
                      ...
                      <td><?= date("M d, Y h:i A", strtotime($log['CreatedAt'])) ?></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
