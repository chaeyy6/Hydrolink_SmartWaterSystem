...
require_once 'phpqrcode/qrlib.php'; // QR code library
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            // Insert new user (without QR first)
            $stmt = $conn->prepare(
                "INSERT INTO users (Name, Email, Contact, PasswordHash, Role) VALUES (?, ?, ?, ?, ?)"
            );
            $stmt->bind_param("sssss", $name, $email, $contact, $hashedPassword, $defaultRole);
            ...
            if ($stmt->execute()) {
                ...
                $uniqueCode = "USR-" . uniqid($newUserId . "-");
                ...
                QRcode::png($uniqueCode, $qrPath, QR_ECLEVEL_L, 5);

                // Save QRCode string (not the image path) into DB
                $stmtUpdate = $conn->prepare("UPDATE users SET QRCode = ? WHERE UserID = ?");
                $stmtUpdate->bind_param("si", $uniqueCode, $newUserId);
                $stmtUpdate->execute();
                $stmtUpdate->close();

                $success = "Registration successful! QR code generated. You can now <a href='login.php'>login here</a>.";

                // Clear form fields
                $name = $email = $contact = "";
            } else {
                $errors[] = "Error: Could not save data.";
            }
        }
        $stmt->close();
    }
...
<head>
    <meta charset="utf-8">
    <title>Hydrolink - Registration Page</title>
    ...
    <div class="container-fluid position-relative p-0">
        ...
        <div class="container-fluid bg-breadcrumb">
            <div class="container text-center py-5" style="max-width: 900px;">
                <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">Register Now</h4>
            </div>
        </div>
        <!-- Header End -->
    </div>
    ...
    <div class="container-fluid bg-light py-5">
        <div class="container py-5">
            <div class="text-center mx-auto pb-5 wow fadeInUp" data-wow-delay="0.2s" style="max-width:800px;">
                <h4 class="text-uppercase text-primary">Create Account</h4>
                <h1 class="display-3 text-capitalize mb-3">Register to Hydrolink</h1>
            </div>
                ...
                <div class="col-lg-6">
                    <form method="POST" action="">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="contact" class="form-control p-3"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="password" name="confirm_password" class="form-control p-3"
                                id="confirm_password">
                        </div>
                        <button type="submit" class="btn btn-primary rounded-pill py-3 px-5">Register</button>
                    </form>
                </div>
            </div>
        </div>
