...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT UserID, Name, PasswordHash, Role FROM users WHERE Email = ?");
        ...
        if ($stmt->num_rows === 1) {
            ...
            if (password_verify($password, $hashedPassword)) {
                ...
                // ✅ Mirror into user_sessions for Python API
                $sessionId = session_id();
                $expiresAt = date('Y-m-d H:i:s', time() + 24 * 60 * 60); // 24h validity

                $stmt2 = $conn->prepare("
                    INSERT INTO user_sessions (SessionID, UserID, ExpiresAt)
                    VALUES (?, ?, ?)
                    ON DUPLICATE KEY UPDATE UserID = VALUES(UserID), ExpiresAt = VALUES(ExpiresAt)
                ");
                $stmt2->bind_param("sis", $sessionId, $userID, $expiresAt);
                $stmt2->execute();
                $stmt2->close();

                // ✅ Check if Terms & Conditions already accepted
                $stmt3 = $conn->prepare("SELECT TermsAccepted FROM users WHERE UserID = ?");
                $stmt3->bind_param("i", $userID);
                $stmt3->execute();
                $stmt3->bind_result($termsAccepted);
                $stmt3->fetch();
                $stmt3->close();

                // === AUDIT TRAIL: Log Admin Login ===
                if ($role === 'Admin') {
                    ...
                    $stmtAudit = $conn->prepare("
                        ...
                        VALUES (?, 'Login', 'Administrator logged in', ?, ?, NOW())
                    ");
                ...
                if (!$termsAccepted) {
                    // Redirect to T&C page if not yet accepteds
                    header("Location: terms_conditions.php");
                    exit;
                }

                // ✅ Redirect based on role (only if T&C already accepted)
                switch ($role) {
                    case 'Admin':
...
<head>
    <meta charset="utf-8">
    <title>Hydrolink - Login Page</title>
    ...
    <div class="container-fluid position-relative p-0">
        ...
        <div class="container-fluid bg-breadcrumb">
            <div class="container text-center py-5" style="max-width: 900px;">
                <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">Log In Now</h4>
            </div>
        </div>
        <!-- Header End -->
    </div>
    ...
    <div class="container-fluid bg-light py-5">
        <div class="container py-5">
            <div class="text-center mx-auto pb-5" style="max-width: 800px;">
                <h4 class="text-uppercase text-primary">Welcome Back</h4>
                <h1 class="display-3 text-capitalize mb-3">Login to Hydrolink</h1>
            </div>
            <div class="row justify-content-center">
                <div class="col-lg-6">
                    <form method="POST" action="">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill py-3 px-5">Login</button>
                    </form>
                </div>
            </div>
        </div>
