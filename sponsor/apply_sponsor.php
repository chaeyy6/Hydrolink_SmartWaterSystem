...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (empty($errors)) {
    $check = $conn->prepare("SELECT ApplicationID FROM RoleApplications WHERE UserID=? AND RoleRequested='Sponsor' AND Status='Pending'");
    $check->bind_param("i", $userID);
    $check->execute();
    $check->store_result();

    if ($check->num_rows > 0) {
      $errors[] = "You already have a pending Sponsor application.";
    }
    $check->close();
  }
  ...
  if (empty($errors)) {
    ...
    $stmt = $conn->prepare("INSERT INTO RoleApplications (UserID, RoleRequested, Notes) VALUES (?, 'Sponsor', ?)");
    $stmt->bind_param("is", $userID, $notes);

    if ($stmt->execute()) {
      $applicationID = $stmt->insert_id; // get the newly created application id
      $stmt->close();

      // 2) Insert extra sponsor details into SponsorApplications
      $stmt2 = $conn->prepare("INSERT INTO SponsorApplications
                (ApplicationID, SponsorName, ContactPerson, ContactNumber, Budget, PreferredArea, AttachmentFile)
                VALUES (?, ?, ?, ?, ?, ?, ?)");
      $stmt2->bind_param("isssdss", $applicationID, $sponsor_name, $contact_person, $contact_number, $budget, $preferred_area, $filePath);

      if ($stmt2->execute()) {
        $success = "Application submitted successfully! You may check your status in My Applications.";

        // Reset CSRF token to prevent reuse
        $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
      } else {
        $errors[] = "Error inserting sponsor details.";
      }
      $stmt2->close();
...
<head>
  <meta charset="utf-8">
  <title>Hydrolink - Apply as a Sponsor Page</title>
...
<body>
  ...
  <div class="container-fluid position-relative p-0">
    ...
    <div class="container-fluid bg-breadcrumb">
      <div class="container text-center py-5" style="max-width: 900px;">
        <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">Apply as a Sponsor</h4>
      </div>
    </div>
    <!-- Header End -->
  </div>
  ...
  <!-- Apply as Sponsor Start -->
  <div class="container-fluid bg-light py-5">
    <div class="container py-5">
      <div class="text-center mx-auto pb-5 wow fadeInUp" data-wow-delay="0.2s" style="max-width: 800px;">
        <h4 class="text-uppercase text-primary">Sponsor Application</h4>
        <h1 class="display-4 text-capitalize mb-3">Apply as a Hydrolink Sponsor</h1>
        <p>Please fill out the form below to submit your request to become a sponsor.</p>
      </div>
      ...
      <div class="row justify-content-center">
        <div class="col-lg-8">
          <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="mb-4">
              <label class="form-label">Organization / Sponsor Name</label>
              <input type="text" name="sponsor_name" class="form-control p-3" placeholder="e.g. BlueDrop Foundation">
            </div>
            ...
            <div class="mb-4">
              <label class="form-label">Intended Monthly Budget (₱)</label>
              <input type="number" min="1" step="0.01" name="budget" class="form-control p-3" placeholder="e.g. 20000">
            </div>

            <div class="mb-4">
              <label class="form-label">Preferred Target Areas / Beneficiaries <small
                  class="text-muted">(optional)</small></label>
              <textarea name="preferred_area" class="form-control p-3" rows="3"
                placeholder="Specify community, city, school, etc…"></textarea>
            </div>

            <div class="mb-4">
              <label class="form-label">Additional Notes / Purpose</label>
              <textarea name="notes" class="form-control p-3" rows="4"
                placeholder="Why do you want to become a sponsor?"></textarea>
            </div>
